...
-- 4. Cleaning up any columns that need fixing
...
-- Inserted only unique records using row_num() filtering
...
-- Double checking for duplicates in your final table
WITH duplicate_check AS (
SELECT *,
ROW_NUMBER() OVER (
PARTITION BY Company, Location_HQ, Industry, Laid_off_count, Percentage, `Date`, Stage, Country, Funds_Raised) AS row_num
...
-- Cleaned up temporary row_num column
ALTER TABLE Tech_layoffs.layoffs_staging2
DROP COLUMN row_num;
...
-- 1. Checking the max values of laid off count and percentage
SELECT MAX(Laid_off_count), MAX(Percentage)
FROM Tech_layoffs.layoffs_staging2
WHERE Percentage IS NOT NULL;

-- Top 5 Companies with the biggest single layoffs
SELECT DISTINCT Company, Laid_off_count
FROM Tech_layoffs.layoffs_staging2
ORDER BY 2 DESC
LIMIT 5;
...
-- Total count of companies that laid off 100% of workforce
...
WITH DATE_CTE AS (
SELECT SUBSTRING(Date, 1, 7) as Dates, SUM(Laid_off_count) AS Total_laid_off -- Using backticks around Date
FROM Tech_layoffs.layoffs_staging2
